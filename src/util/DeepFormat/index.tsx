...
export default class DeepFormat {
    ...
    static replace = (
        ...
        const overloads = child?.props?.['apollo-overload'];
        if (overloads) {
            const apolloRef: ApolloRef = { parentProps, componentMap };

            return <Fragment key={key}>{cloneElement(child, { apolloRef })}</Fragment>;
        }
